...
This plugin adds two tools from the hosted Telem search router, surfaced in Claude
Code as `mcp__plugin_telem_telem__telem_search` and
`mcp__plugin_telem_telem__telem_fetch`. This skill is belt-and-suspenders with the
tool descriptions themselves — it restates the preference and the session contract so
...
Passing the session id back is the model-level contract above: it is what keeps one
task's searches on one session and the goal from being re-declared. A PreToolUse hook
shipped with this plugin also sends query lineage with each call, so a whole session's
searches stay grouped even if a session id is forgotten — that is a backstop, not a
reason to skip the contract.

## Never set `telem_bridge`

`telem_search` and `telem_fetch` accept a `telem_bridge` parameter. It is reserved for
the Telem host bridge — never set it. The plugin's hook fills it in.
